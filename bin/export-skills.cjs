...
var k = __toESM(require("node:readline"), 1);
...
var B = class {
  ...
  constructor(e, s = true) {
    const { input: i = import_node_process.stdin, output: r = import_node_process.stdout, render: n, signal: u, ...a } = e;
...
${(0, import_node_util2.styleText)("gray", x)}  ` : "";
  s.write(`${i}${e}

`);
};
var Kt = (e) => (0, import_node_util2.styleText)("magenta", e);
...
`);
    ...
    g = true, p = xt({ output: s }), $ = T2(B2), d = performance.now(), O2 && s.write(`${(0, import_node_util2.styleText)("gray", h)}
`);
    let P2 = 0, G2 = 0;
    b(), f = setInterval(() => {
      if (c && $ === m) return;
    ...
    const L2 = P2 === 0 ? (0, import_node_util2.styleText)("green", V) : P2 === 1 ? (0, import_node_util2.styleText)("red", $e) : (0, import_node_util2.styleText)("red", de);
...
var Zt = (e) => new $t({ validate: e.validate, placeholder: e.placeholder, defaultValue: e.defaultValue, initialValue: e.initialValue, output: e.output, signal: e.signal, input: e.input, render() {
  const r = e?.withGuide ?? _.withGuide, s = `${`${r ? `${(0, import_node_util2.styleText)("gray", h)}
` : ""}${W2(this.state)}  `}${e.message}
`, i = e.placeholder ? (0, import_node_util2.styleText)("inverse", e.placeholder[0]) + (0, import_node_util2.styleText)("dim", e.placeholder.slice(1)) : (0, import_node_util2.styleText)(["inverse", "hidden"], "_"), a = this.userInput ? this.userInputWithCursor : i, o = this.value ?? "";
  switch (this.state) {
    case "error": {
      const u = this.error ? `  ${(0, import_node_util2.styleText)("yellow", this.error)}` : "", l = r ? `${(0, import_node_util2.styleText)("yellow", h)}  ` : "", n = r ? (0, import_node_util2.styleText)("yellow", x) : "";
      return `${s.trim()}
${l}${a}
${n}${u}
`;
...
${l}` : ""}`;
    ...
    default: {
      const u = r ? `${(0, import_node_util2.styleText)("cyan", h)}  ` : "", l = r ? (0, import_node_util2.styleText)("cyan", x) : "";
      return `${s}${u}${a}
${l}
`;
    }
...
// scripts/export-skills.ts
var agentSkills = pgTable("agent_skills", {
...
async function main() {
  console.clear();
  Wt2(import_picocolors.default.bgCyan(import_picocolors.default.black(" Agent Skills Exporter ")));
  let dbUrl = process.env.DATABASE_URL;
  if (!dbUrl) {
    const userInput = await Zt({
    ...
    if (typeof userInput !== "string") {
      Gt(import_picocolors.default.yellow("Cancelled."));
      process.exit(0);
    }
    dbUrl = userInput;
  }
  const exportDir = await Zt({
  ...
  if (typeof exportDir !== "string" || !exportDir) {
    Gt(import_picocolors.default.yellow("Export cancelled."));
    process.exit(0);
  }
  const s = be();
  s.start("Connecting to database \u{1F4E1}");
  const pool = new Pool({
  ...
  try {
    ...
    s.stop(`Found ${import_picocolors.default.cyan(skills.length)} skills in the database.`);
    if (skills.length === 0) {
      Gt(import_picocolors.default.yellow("No skills found to export."));
      process.exit(0);
    }
    const outPath = import_path.default.resolve(process.cwd(), exportDir);
    s.start(`Creating directory ${outPath}`);
    await import_promises.default.mkdir(outPath, { recursive: true });
    s.stop("Directory ready.");
    s.start("Exporting skills to Markdown...");
    let exportedCount = 0;
    for (const skill of skills) {
      const filePath = import_path.default.join(outPath, `${skill.slug}.md`);
      let frontmatter = "---\n";
...
`;
    ...
    Gt(import_picocolors.default.green(`All done! Skills saved in ${exportDir}/`));
  } catch (err) {
...
main().catch(() => {
  Gt(import_picocolors.default.red("An unexpected error occurred."));
  process.exit(1);
});
